...
int output_frames(struct h264_dpb_stru *p_H264_Dpb, unsigned char flush_flag)
{
	...
	p_H264_Dpb->frame_done_cb(p_H264_Dpb->sess, p_Dpb->fs[pos]);
...
{
	struct DecodedPictureBuffer *p_Dpb = &p_H264_Dpb->mDPB;
	struct FrameStore *frame = NULL;
	int i;

	for (i = 0; i < DPB_SIZE_MAX; i++) {
		if (p_H264_Dpb->mFrameStore[i].buf_spec_num == index &&
		    p_H264_Dpb->mFrameStore[i].pre_output) {
			if (frame) {
				printk("Double frame!\n");
				amvdec_abort(p_H264_Dpb->sess);
				return;
			}
			frame = &p_H264_Dpb->mFrameStore[i];
		}
	}

	if (!frame) {
		printk("Couldn't find frame with buf %d\n", index);
		amvdec_abort(p_H264_Dpb->sess);
		return;
	}
...
static void get_free_buf(struct amvdec_session *sess, struct StorablePicture *pic)
{
	...
	printk("pic %d <-> buf %d\n", pic->index, vbuf->vb2_buf.index);
	pic->vbuf = vbuf;
	pic->buf_spec_num = vbuf->vb2_buf.index;
}
	...
	if (new_pic_flag) { /* new picture */
		...
		if (p_H264_Dpb->mVideo.dec_picture) {
			...
			if (p_H264_Dpb->mVideo.dec_picture->buf_spec_num ==
				-1) {
				get_free_buf(p_H264_Dpb->sess,
					     p_H264_Dpb->mVideo.dec_picture);
				/*p_H264_Dpb->mVideo.dec_picture->buf_spec_num =
					get_free_buf_idx(p_H264_Dpb->sess);*/
				if (p_H264_Dpb->mVideo.dec_picture->buf_spec_num
					< 0) {
					p_H264_Dpb->buf_alloc_fail = 1;
					p_H264_Dpb->mVideo.dec_picture->
